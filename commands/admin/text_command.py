...
async def take_the_money(message: types.Message):
    ...
    try:
        ...
        await message.answer(f'{url}, чтобы забрать деньги нужно ответить на сообщение пользователя.')
        return
...
async def reset_the_money(message: types.Message):
    ...
    try:
        ...
        await message.answer(f'{url}, чтобы обнулить игрока нужно ответить на сообщение пользователя.')
        return
