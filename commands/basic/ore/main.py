...
async def buy_btc_cmd(message: types.Message, user: BFGuser):
    ...
    try:
        summ_btc = message.text.split()[2].replace('е', 'e')
        summ_btc = int(float(summ_btc))
    except:
    ...
    if summ_btc <= 0:
        await message.answer(f'{user.url}, нельзя покупать отрицательное или нулевое количество BTC {lose}')
        return
