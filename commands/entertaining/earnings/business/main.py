...
async def upd_business_text(call: types.CallbackQuery | types.Message, user: BFGuser, action='edit'):
    ...
    dox = int(15000000000 * business.bsterritory.get() / 100)
...
@antispam_earning
async def show_income(call: types.CallbackQuery, user: BFGuser):
    win, lose = BFGconst.emj()
    business = user.business
    
    if not business:
        await call.answer(f'{user.name}, у вас нет своего бизнеса чтобы построить введите команду "Построить бизнес" {lose}')
        return
    
    dox = int(15000000000 * business.bsterritory.get() / 100)
    
    txt = f'''{user.url}, ваш доход:
💷 Ваш текущий доход бизнеса: {tr(dox)}$
'''
...
async def sell_business(message: types.Message, user: BFGuser):
    ...
    for i in range(6, business.territory.get() + 1):
        summ += int(22_000_000 * (1 + 0.15) ** (i - 4)) // 2
        
    for i in range(6, business.bsterritory.get() + 1):
...
def reg(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(show_income, text_startswith='business-income')
