...
def business(user_id):
    ...
    buttons = [
        ...
        InlineKeyboardButton("💴 Текущий доход (В разработке)", callback_data=f"В разработке...|{user_id}"),
    ]
    keyboards.add(buttons[0], buttons[1])
    keyboards.add(buttons[2], buttons[3])
    keyboards.add(buttons[4])
    return keyboards
...
def quarry(user_id):
    ...
    buttons = [
        ...
        InlineKeyboardButton("📦 Показать доход", callback_data=f'business-income-{user_id}')
    ]
    keyboards.add(buttons[0], buttons[1])
    keyboards.add(buttons[2], buttons[3])
    return keyboards
...
def clan(user_id):
    ...
    buttons = [
        ...
        InlineKeyboardButton("🛠 Настройки", callback_data=f"clan-settings|{user_id}"),
    ]
    print(3444444444)
    keyboards.add(buttons[0], buttons[1])
    keyboards.add(buttons[2])
